...
type CreateProductRequestBody =
  NonNullable<paths["/api/products"]["post"]["requestBody"]>["content"]["application/json"];

type CreateProduct201Response =
  paths["/api/products"]["post"]["responses"]["201"]["content"]["application/json"];

type UpdateProductRequestBody =
  NonNullable<paths["/api/products/{productId}"]["put"]["requestBody"]>["content"]["application/json"];
...
export async function updateProductApiRequest(
  params: { productId: string } & UpdateProductRequestBody & { idempotencyKeyOptional?: string }
) {
  return httpRequestJson<UpdateProduct200Response>(`/api/products/${params.productId}`, {
    method: "PUT",
    body: JSON.stringify({
      ...(params.productName !== undefined && { productName: params.productName }),
      ...(params.productPriceCents !== undefined && { productPriceCents: params.productPriceCents }),
...
export async function listProductsApiRequest(params?: { limit?: number; cursorId?: string }) {
  ...
  return httpRequestJson<ListProducts200Response>(`/api/products${qs ? `?${qs}` : ""}`);
}

export async function deleteProductApiRequest(params: { productId: string; idempotencyKeyOptional?: string }) {
  return httpRequestJson<DeleteProduct200Response>(`/api/products/${params.productId}`, {
    method: 'DELETE',
    headers: params.idempotencyKeyOptional
      ? { 'Idempotency-Key': params.idempotencyKeyOptional }
      : undefined,
