...
const LOCALSTORAGE_LIMIT_KEY = "theme-activity:perPage";

// Keys we manage in the query string
const OWN_KEYS = [
  "mode",
  "limit",
  "page",
...
export default function ThemeActivityTab() {
  ...
  const [rows, setRows] = useState<ThemeActivityItem[] | null>(null);
  ...
  // Simple per-tenant facet cache to reduce flicker
  ...
  function writeUrlState(overrides?: {
    ...
    // Merge with existing params so ?tab=activity survives
    const sp = new URLSearchParams(searchParams);

    // Remove our keys, rewrite cleanly
  ...
  async function fetchPage(opts?: {
    ...
    const justPaginating = opts?.keepFacets === true && !opts?.includeTotal;
    !justPaginating && setIsLoading(true);
    justPaginating && setIsPaginating(true);
    ...
    try {
      ...
      const res = await getTenantThemeActivityApiRequest({
        tenantSlug,
      ...
      const items = (data.items ?? []) as ThemeActivityItem[];
      setRows(items);
      ...
      if (!opts?.keepFacets) {
        ...
        if (fromServer) {
          ...
          facetCache.current.set(tenantSlug, { actors: mapped });
          setActorOptions(mapped);
        } else {
          const cached = facetCache.current.get(tenantSlug)?.actors;
      ...
      justPaginating ? setIsPaginating(false) : setIsLoading(false);
    }
  }
  ...
  function resetToFirstPageAndFetch(opts?: { keepFacets?: boolean; limitOverride?: number }) {
    ...
    void fetchPage({ cursor: null, keepFacets: opts?.keepFacets, limitOverride: opts?.limitOverride, includeTotal: true });
  }
  ...
  useEffect(() => {
    ...
    const cached = tenantSlug ? facetCache.current.get(tenantSlug)?.actors : undefined;
    if (cached) setActorOptions(cached);
    ...
    <Group justify="space-between" align="start">
      ...
      <Group gap="sm" align="center">
        ...
        <Button
          ...
          aria-controls="theme-activity-filters"
        >
          Filters
        </Button>
        <Button
  ...
  const filterPanel = (
    <>
      <FilterBar<Filters>
        open={showFilters}
        panelId="theme-activity-filters"
      ...
      <Paper withBorder p="md" radius="md" className="bg-white">
        ...
        <div className="py-16 text-center">
          ...
          <Group justify="center">
            <Button variant="light" onClick={() => setShowFilters(true)} aria-controls="theme-activity-filters" aria-expanded={showFilters}>
              Show filters
            </Button>
  ...
  // Tiny overlay during pagination
  const Overlay = () =>
    isPaginating ? (
    ...
    <>
      ...
      <Paper withBorder p="md" radius="md" className="bg-white max-h-[70vh] overflow-y-auto">
        <Group justify="space-between" align="center" gap="xs" wrap="wrap">
          ...
          <Group align="center" gap="xs">
            ...
            <NumberInput
              ...
              aria-controls="theme-activity-table"
            />
          </Group>
        </Group>
        ...
        <div className="relative">
          <Overlay />
          <Table id="theme-activity-table" striped withTableBorder withColumnBorders stickyHeader>
            <Table.Thead>
              <Table.Tr>
